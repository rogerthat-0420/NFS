...
void *client_listen_thread_function()
{
    ...
    printf(BLUE "Listening for instructions from clients on Port: %d\n" RESET, ss_info.client_port);
...
int delete_file_or_directory(const char *name, int is_directory)
{
    ...
    if (is_directory)
    {
        ...
        printf("Error: execvp failed to execute the delete command.\n");
    }
    else
    {
