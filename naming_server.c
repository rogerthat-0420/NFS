...
void *failure_detection_thread(void *args)
{
    ...
    while (1)
    {
        ...
        if (send(sockfd, "PING", 4, 0) <= 0)
        {
            ...
            printf(GREEN "[+] Server %d disconnected\n" RESET, num + 1);
            connected[num] = 0;
        ...
        {
            pthread_mutex_lock(&lock_connected);
            printf(RED "[-] Server %d disconnected\n" RESET, num + 1);
            connected[num] = 0;
            pthread_mutex_unlock(&lock_connected);
            pthread_exit(NULL);
        }
...
void *ss_thread_function(void *args)
{
    ...
    for (int i = 0; i < num_ss; i++)
    {
        ...
        if (list_of_ss[i].nm_port == ss_info.nm_port && list_of_ss[i].client_port == ss_info.client_port)
        {
            ...
            connected[i] = 1;
            pthread_mutex_unlock(&lock_connected);
            printf(GREEN "[+] Server %d back online\n" RESET, i + 1);
...
void *alt_listen_ss_function()
{
    ...
    {
        perror(RED "SS_Binding failed" RESET);
        close(server_socket);
        exit(EXIT_FAILURE);
    }
    ...
    printf(BLUE "NM listening for Storage Servers on Port: %d\n" RESET, NM_SS_PORT);
    ...
    while (1)
    {
        ...
        max_ss_till_now++;
        printf(GREEN "[+] Server %d connected\n" RESET, num_ss + 1);
        connected[num_ss] = 1;
        int index = num_ss;
        args.index = index;
        args.socket_fd = ss_socket;
...
void *listen_cl_function()
{
    ...
    if (bind(server_socket, (struct sockaddr *)&server_address, sizeof(server_address)) == -1)
    {
        perror(RED "CL_Binding failed" RESET);
        close(server_socket);
        return NULL;
    }
    ...
    printf(BLUE "NM listening for Clients on Port: %d\n" RESET, NM_CL_PORT);
