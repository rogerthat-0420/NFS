...
void communicate_with_ss(Server_Info *ss_info, File_Request *request, char *write_data)
{
    ...
    else
    {
        ...
        printf(YELLOW "Size = %lld\n" RESET, file_info->size);
        printf(YELLOW "Permissions = %s\n" RESET, file_info->permissions);
    }
}
...
int main()
{
    ...
    while (1)
    {
        ...
        if (strcmp("READ", operation) != 0 && strcmp("WRITE", operation) != 0 && strcmp("CREATE", operation) != 0 && strcmp("DELETE", operation) != 0 && strcmp("COPY", operation) != 0 &&
